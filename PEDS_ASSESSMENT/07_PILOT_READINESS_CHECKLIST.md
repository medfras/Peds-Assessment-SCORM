...
- [x] SCORM pass challenge gate implemented: 2 PM1 + 2 PT1 passing/on-track scenarios + ≥3600 s training time from orientation/scenarios/drills + ≥1200 XP
...
- [ ] **Raw transcript retention policy:** Keep the current 30-day purge for raw learner free-text/transcript data (`chat_messages`, `SessionFinding`, `dmist_report`, and `narrative_data["narrative"]`). Before production rollout, verify the daily TTL worker is running in Render logs (`ttl_scrub.complete` / `ttl_scrub.error`) and document how pilot operators can confirm purge activity.
- [ ] **SessionEvent retention decision:** Resolve the current policy mismatch before production rollout: `docs/DEVELOPMENT_GUIDELINES.md` says `SessionEvent` rows are scrubbed by the 30-day TTL worker, but the current worker retains them. Decide whether structured `SessionEvent` rows should be retained for audit/scoring review or scrubbed with raw transcript data, then update either the worker or the docs so the retention policy is accurate.
- [ ] **Evidence-packet privacy review:** Confirm whether `sessions.evidence_packet`, `sessions.feedback`, and `sessions.debrief_markdown` can contain learner-entered free text. If yes, define a time-limited scrub/archive policy aligned with the 30-day raw transcript purge; if no, document why these fields are safe to retain longer for scoring audit and learner history.
