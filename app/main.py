...
async def get_my_sessions(
    ...
    def _session_dict(s: SimSession) -> dict:
        ...
        return {
            ...
            "scoreNotes":      nd.get("score_notes") or {},
...
def _build_session_timeline(
    ...
    for rec in recommended_actions:
        ...
        # Pediatric head-injury pilot rule: an initial formal neuro/pupil exam is
        # required and scored, but repeat GCS+pupil trending is not a required
        # debrief/timeline item for these non-transport scenario reps.
        if rec_id == "reassess_neuro" or ("gcs" in desc_lower and "pupil" in desc_lower):
            continue
...
async def submit_narrative(
    ...
    if session.narrative_submitted and session.feedback:
        ...
        # Timeline/rubric rows are deterministic display artifacts; rebuild them
        # from current checklist evidence instead of reusing stale narrative_data
        # generated by older timeline logic.
        #
        # Re-run adjudication first. Scenario checklist/timeline fixes can change
        ...
        _cached_rubric_detail = _build_session_rubric_detail(session)
        if stored.get("timeline") != _cached_timeline or stored.get("rubric_detail") != _cached_rubric_detail:
            stored["timeline"] = _cached_timeline
            stored["rubric_detail"] = _cached_rubric_detail
            if _det_packet_cached is not None:
                det = extract_deterministic_subscores(_det_packet_cached)
                _cached_subscores = dict(stored.get("subscores") or {})
                for cat, val in det.items():
                    if val is not None:
                        _cached_subscores[cat] = val
                maxes = dict(_cached_subscores.get("_maxes") or {})
                for cat, score_row in _det_packet_cached.score_snapshot.items():
                    if score_row.total is not None:
                        maxes[cat] = int(score_row.max)
                if maxes:
                    _cached_subscores["_maxes"] = maxes
                stored["subscores"] = _cached_subscores
            session.narrative_data = stored
            flag_modified(session, "narrative_data")
            await db.commit()
        ...
        return {
            ...
            "score_notes":               stored.get("score_notes") or {},
            "timeline":                  _cached_timeline,
            "rubric_detail":             _cached_rubric_detail,
...
async def skip_narrative(
    ...
    if session.narrative_submitted and session.feedback:
        # Recompute deterministic display artifacts on cached responses so a
        # session generated under older timeline logic does not keep replaying
        # stale missed/applied rows after backend scoring fixes.
        #
        ...
        _cached_skip_rubric_detail = _build_session_rubric_detail(session)
        if stored.get("timeline") != _cached_skip_timeline or stored.get("rubric_detail") != _cached_skip_rubric_detail:
            stored["timeline"] = _cached_skip_timeline
            stored["rubric_detail"] = _cached_skip_rubric_detail
            if _det_packet_cached_skip is not None:
                det = extract_deterministic_subscores(_det_packet_cached_skip)
                _cached_skip_subscores = dict(stored.get("subscores") or {})
                for cat, val in det.items():
                    if val is not None:
                        _cached_skip_subscores[cat] = val
                maxes = dict(_cached_skip_subscores.get("_maxes") or {})
                for cat, score_row in _det_packet_cached_skip.score_snapshot.items():
                    if score_row.total is not None:
                        maxes[cat] = int(score_row.max)
                if maxes:
                    _cached_skip_subscores["_maxes"] = maxes
                stored["subscores"] = _cached_skip_subscores
            session.narrative_data = stored
            flag_modified(session, "narrative_data")
            await db.commit()
        ...
        return {
            ...
            "subscores":                _eff_subscores or None,
            "score_notes":              stored.get("score_notes") or {},
            "timeline":                 _cached_skip_timeline,
            "rubric_detail":            _cached_skip_rubric_detail,
