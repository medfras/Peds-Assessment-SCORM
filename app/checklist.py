...
_BASE_RUBRIC_REGISTRY: dict[str, list[dict[str, Any]]] = {
    ...
    "nremt_trauma_v1": [
        ...
        _base_item("ems.trauma.airway", "Airway opened/assessed and adjunct inserted as indicated", 2, _TRAUMA_CATS, tier2_patterns=[r"(?i)(airway|jaw thrust|open.*airway|OPA|NPA|adjunct|suction)"], critical_failure=True, critical_failure_label="Failure to manage airway threats"),
        ...
        _base_item("ems.trauma.priority_transport", "Identifies patient priority and makes treatment/transport decision based on GCS", 1, _TRAUMA_CATS, subtype="transport", tier2_patterns=[r"(?i)(priority|transport|rapid transport|load and go|GCS|trauma center|ALS|hospital|unstable)"], applicable_if={"non_transport_agency": False}, critical_failure=True, critical_failure_label="Failure to differentiate immediate transport need versus continued assessment/treatment"),
        ...
        _base_item("ems.trauma.head_scalp_ears", "Head assessment: inspects and palpates scalp and ears", 1, _TRAUMA_CATS, tier2_patterns=[r"(?i)(assess|inspect|palpat|check|examin|look at|evaluate).{0,40}(head|scalp|ears?)|(head|scalp|ears?).{0,40}(assessment|exam|inspection|palpation|checked|clear|normal|abnormal|DCAP)"]),
        _base_item("ems.trauma.head_eyes", "Head assessment: assesses eyes", 1, _TRAUMA_CATS, tier1_match={"source": "finding", "finding_key_pattern": r"(?i)(pupils?|eyes?)"}, tier2_patterns=[r"(?i)(assess|inspect|check|examin|look at|evaluate).{0,40}(eyes?|pupils?)|(eyes?|pupils?).{0,40}(assessment|exam|checked|PERRL|equal|reactive)"]),
        _base_item("ems.trauma.head_mouth_nose_face", "Head assessment: inspects mouth, nose, and facial area", 1, _TRAUMA_CATS, tier2_patterns=[r"(?i)(assess|inspect|check|examin|look at|evaluate).{0,40}(mouth|nose|face|facial)|(mouth|nose|face|facial).{0,40}(assessment|exam|inspection|checked|clear|normal|abnormal)"]),
        _base_item("ems.trauma.neck_trachea", "Neck assessment: checks position of trachea", 1, _TRAUMA_CATS, tier2_patterns=[r"(?i)(trachea|tracheal)"]),
        _base_item("ems.trauma.neck_jugular_veins", "Neck assessment: checks jugular veins", 1, _TRAUMA_CATS, tier2_patterns=[r"(?i)(JVD|jugular|neck veins?)"]),
        _base_item("ems.trauma.neck_c_spine", "Neck assessment: palpates cervical spine", 1, _TRAUMA_CATS, tier2_patterns=[r"(?i)(cervical|c.?spine|neck).{0,40}(palpat|tender|pain|step.?off|DCAP)|(palpat|assess|check|examin).{0,40}(cervical|c.?spine|neck)"]),
        _base_item("ems.trauma.chest_inspect", "Chest assessment: inspects chest", 1, _TRAUMA_CATS, tier2_patterns=[r"(?i)(inspect|look at|assess|examin|check).{0,40}(chest|thorax)|(chest|thorax).{0,40}(inspection|inspected|looked|clear|normal|abnormal|DCAP)"]),
        _base_item("ems.trauma.chest_palpate", "Chest assessment: palpates chest", 1, _TRAUMA_CATS, tier2_patterns=[r"(?i)(palpat|feel).{0,40}(chest|thorax)|(chest|thorax).{0,40}(palpat|tender|crepitus|instability|DCAP)"]),
        ...
        _base_item("ems.trauma.abdomen_inspect_palpate", "Abdomen/pelvis assessment: inspects and palpates abdomen", 1, _TRAUMA_CATS, tier2_patterns=[r"(?i)(inspect|palpat|assess|check|examin).{0,40}(abdomen|belly)|(abdomen|belly).{0,40}(inspect|palpat|tender|rigid|soft|DCAP)"]),
        _base_item("ems.trauma.pelvis_assess", "Abdomen/pelvis assessment: assesses pelvis", 1, _TRAUMA_CATS, tier2_patterns=[r"(?i)(pelvis|pelvic)"]),
