...
def test_drill_launch_routes_are_distinct_for_gcs_cpr_and_ams():
    ...
    assert '<script src="/static/js/app.js?v=20260630-debrief-contrast-v4"></script>' in html
...
def test_station1_cpr_training_node_is_labeled_as_drill():
    ...
    assert "const previousScores = _progressCache?.minigameBestScores || {};" not in js
    assert "minigameBestScores: mergedScores" not in js
