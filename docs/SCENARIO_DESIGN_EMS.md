...
Pediatric length-based tape references are derived deterministically from `patient.weight_kg` during scenario adaptation. Do not ask the LLM to infer Broselow/length-based tape color, weight range, age range, or measurement from memory. The application default is the Michigan length-based tape table for the pilot, and local/state variation belongs in agency config under `pediatric_length_based_tape` using either full `bands` replacement or per-color `band_overrides`.
